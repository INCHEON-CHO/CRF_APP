...
parser.add_argument('--checkpoint_dir', default='..\\..\\model_logs\\model\\snap-60000', type=str,
...
def getFile():
    if request.method == 'POST':
        ...
        with graph.as_default():
            set_session(sess)
            start = time.time()
            filename = os.getcwd() + "\\original.jpg"
            image = skimage.io.imread(filename)
            results = model.detect([image], verbose=1)
            
            r = results[0]
            save_image(image,"results", r['rois'], r['masks'],r['class_ids'],r['scores'],class_names,filter_classs_names=['person'],scores_thresh=0.9,save_dir=".\\",mode=0)
